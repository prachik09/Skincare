...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['product_name'])) {
    ...
    if (isset($_FILES['image']) && $_FILES['image']['error'] == 0) {
        ...
        if (move_uploaded_file($image_tmp_name, $target_file)) {
            $sql = "INSERT INTO products (name, price, description, tags, brand, image) VALUES (?, ?, ?, ?, ?, ?)";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param("sdssss", $productName, $price, $description, $tags, $brand, $target_file);
...
if (isset($_POST['update_product'])) {
    ...
    if ($imagePath) {
        $sql = "UPDATE products SET name=?, price=?, description=?, tags=?, brand=?, image=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sdssssi", $productName, $price, $description, $tags, $brand, $imagePath, $productId);
    } else {
        $sql = "UPDATE products SET name=?, price=?, description=?, tags=?, brand=? WHERE id=?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sdsssi", $productName, $price, $description, $tags, $brand, $productId);
    }
...
form input[type="file"] {
    margin: 5px 0 10px;
    width: 80%; /* Input fields now take up 80% of the parent width */
}
